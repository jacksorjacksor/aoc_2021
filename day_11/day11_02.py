...
while True:
    ...
    flash_counter_this_step = 0
    for octopus in list_of_dumbo_octopus:
        if octopus.has_flashed_this_step:
            flash_counter_this_step += 1
            octopus.has_flashed_this_step = False
            octopus.energy_level = 0

    print(f"{turn_counter} |{flash_counter_this_step}")

    if flash_counter_this_step == 100:
        print(f"DONE: {turn_counter} | {flash_counter}")
        break
